...
class LocalJsonRepo:
    ...
    def delete_clinic(self, clinic_id: str) -> None:
        rows = [c for c in self._read("clinics") if c.get("id") != clinic_id]
        self._write("clinics", rows)
        # cascade delete contacts, visits, notes
        contacts = [x for x in self._read("contacts") if x.get("clinic_id") != clinic_id]
        self._write("contacts", contacts)
        visits = [v for v in self._read("visits") if v.get("clinic_id") != clinic_id]
        self._write("visits", visits)
        # remove notes by visits
        visit_ids = {v.get("id") for v in visits}
        notes = [n for n in self._read("notes") if n.get("visit_id") in visit_ids]
    ...
    def delete_contact(self, contact_id: str) -> None:
        rows = [c for c in self._read("contacts") if c.get("id") != contact_id]
        self._write("contacts", rows)
    ...
    def delete_visit(self, visit_id: str) -> None:
        visits = [v for v in self._read("visits") if v.get("id") != visit_id]
        self._write("visits", visits)
        notes = [n for n in self._read("notes") if n.get("visit_id") != visit_id]
        self._write("notes", notes)
